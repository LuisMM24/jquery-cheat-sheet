...
function vanillaFunctions(){
    ...
    function htmlDocumentLoadedVanilla(){
        p.innerText="The document is loaded!"
        demo.appendChild(p)
    }
    ...
    function changeValueSelectVanilla(){
        ...
        select.addEventListener("change",()=>{
                p.innerText=`you change the option to ${select.value}!`
                demo.appendChild(p)
        })
    ...
    function getParentHtmlElementVanilla(){
        ...
        p.innerText="Im te child"
        exampleBtn.addEventListener("click", function(){
            p.parentNode.style.fontWeight="bolder";
        })
        demo.removeChild(demo.lastChild);
        demo.appendChild(exampleBtn);
    ...
    function getOptionsVanilla(){
        ...
        select.addEventListener("change",()=>{
            if(select.selectedOptions.length==2){
                p.innerText+=`, the${select.selectedOptions[1].value}`
            }else if(select.selectedOptions.length==3){
                p.innerText+=`, and the${select.selectedOptions[2].value}`
            }else{
                p.innerText=`you picked the option ${select.selectedOptions[0].value}`
            }
            demo.appendChild(p)
        })
        demo.appendChild(select)
